...
function BeforeSleepiness({ onBack, onComplete }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/v1/sleepiness/out`,
        {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({
            beforeSleepiness: selectedValue
          })
        }
      );
      ...
      if (!response.ok) {
        throw new Error(
          result.message || "졸림 수치 기록에 실패했습니다."
        );
      }

      console.log("외출 전 졸림 기록 성공:", result);

      onComplete(result.recordId);
    } catch (error) {
      console.error("외출 전 졸림 기록 실패:", error);
      alert("졸림 수치를 저장하지 못했습니다. 다시 시도해주세요.");
    } finally {
  ...
  return (
    <div className="sleepiness-page">
      ...
      >
        {isSubmitting ? "기록 중..." : "활동 시작하기"}
      </button>
    </div>
  );
}

export default BeforeSleepiness;